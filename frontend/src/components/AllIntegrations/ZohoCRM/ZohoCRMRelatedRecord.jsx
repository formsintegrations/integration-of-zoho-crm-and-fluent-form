    ...
    <>
      ...
      {!isPro && (
        <div className="pos-rel">
          ...
          {/* // )} */}
          <b className="wdt-100 d-in-b">{__('Related List:', 'bitffzc')}</b>
          <select onChange={handleInput} name="module" value={crmConf?.relatedlists?.[tab - 1]?.module} className="btcd-paper-inp w-7" disabled={!crmConf.module}>
            <option value="">{__('Select Related Module', 'bitffzc')}</option>
            {
          ...
          <button className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh CRM Related Lists', 'bitffzc')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
          <br />
          <br />
          <b className="wdt-100 d-in-b">{__('Layout:', 'bitffzc')}</b>
          <select onChange={handleInput} name="layout" value={crmConf?.relatedlists?.[tab - 1]?.layout} className="btcd-paper-inp w-7" disabled={!crmConf?.relatedlists?.[tab - 1]?.module}>
            <option value="">{__('Select Layout', 'bitffzc')}</option>
            {
      ...
      <br />
      {
        ...
        && (
          <>
            <div className="mt-4"><b className="wdt-100">{__('Field Map', 'bitffzc')}</b></div>
            <div className="btcd-hr mt-1" />
            <div className="flx flx-around mt-2 mb-1">
              <div className="txt-dp"><b>{__('Form Fields', 'bitffzc')}</b></div>
              <div className="txt-dp"><b>{__('Zoho Fields', 'bitffzc')}</b></div>
            </div>
            ...
            {crmConf.default?.layouts[crmConf.relatedlists[tab - 1].module]?.[crmConf.relatedlists[tab - 1].layout] && Object.keys(crmConf.default.layouts[crmConf.relatedlists[tab - 1].module][crmConf.relatedlists[tab - 1].layout].fileUploadFields).length !== 0 && (
              <>
                <div className="mt-4"><b className="wdt-100">{__('File Upload Field Map', 'bitffzc')}</b></div>
                <div className="btcd-hr mt-1" />
                <div className="flx flx-around mt-2 mb-1">
                  <div className="txt-dp"><b>{__('Form Fields', 'bitffzc')}</b></div>
                  <div className="txt-dp"><b>{__('Zoho Fields', 'bitffzc')}</b></div>
            ...
            <div className="mt-4"><b className="wdt-100">{__('Actions', 'bitffzc')}</b></div>
            <div className="btcd-hr mt-1" />
