...
export default function ZohoCRMAuthorization({
  ...
  return (
    ...
    >
      <div className="mt-3">
        <b>{__('Integration Name:', 'bitffzc')}</b>
      </div>
      <input
      ...
      <div className="mt-3">
        <b>{__('Authorized Redirect URIs:', 'bitffzc')}</b>
      </div>
      <CopyText
        value={redirectLocation || bitffzc?.redirect}
        setSnackbar={setSnackbar}
        className="field-key-cpy w-6 ml-0"
        readOnly={isInfo}
      />
      ...
      <small className="d-blk mt-5">
        ...
        <a
          ...
          rel="noreferrer"
        >
          {__('Zoho API Console', 'bitffzc')}
        </a>
      </small>

      <div className="mt-3">
        <b>{__('Client id:', 'bitffzc')}</b>
      </div>
      <input
        className="btcd-paper-inp w-6 mt-1"
        onChange={handleInput}
        ...
        type="text"
        placeholder={__('Client id...', 'bitffzc')}
        disabled={isInfo}
      />
      <div style={{ color: 'red' }}>{error.clientId}</div>

      <div className="mt-3">
        <b>{__('Client secret:', 'bitffzc')}</b>
      </div>
      <input
        className="btcd-paper-inp w-6 mt-1"
        onChange={handleInput}
        ...
        type="text"
        placeholder={__('Client secret...', 'bitffzc')}
        disabled={isInfo}
      />
      ...
      {!isInfo && (
        <>
          <button
            onClick={() =>
              handleAuthorize(crmConf, setCrmConf, setError, setisAuthorized, setisLoading, setSnackbar)
            }
