...
function ActionBlock({ formFields, fields, action, lgcGrpInd, actionInd, setworkFlows, actionType }) {
  ...
  return (
    <div className="flx pos-rel btcd-logic-blk">
      ...
      >
        <option value="">{__('Select One', 'bitffzc')}</option>
        <option value="value">{__('Value', 'bitffzc')}</option>
        {actionType !== 'onsubmit' && <option value="disable">{__('Disable', 'bitffzc')}</option>}
        {actionType !== 'onsubmit' && <option value="readonly">{__('Readonly', 'bitffzc')}</option>}
        {actionType !== 'onsubmit' && <option value="enable">{__('Enable', 'bitffzc')}</option>}
        {actionType !== 'onsubmit' && <option value="hide">{__('Hide', 'bitffzc')}</option>}
        {actionType !== 'onsubmit' && <option value="show">{__('Show', 'bitffzc')}</option>}
      </MtSelect>
