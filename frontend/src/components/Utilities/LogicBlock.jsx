  ...
  return (
    <div className="flx pos-rel btcd-logic-blk">
      ...
      >
        ...
        {formFields.map(
          itm =>
            !itm.type.match(/^(file-up|recaptcha)$/) && (
              <option key={`ff-lb-${itm.key}`} value={itm.key}>
                {itm.name}
              </option>
            )
        )}
      ...
      >
        ...
        {!type.match(/^(date|time|datetime|month|week)$/) && (
          <option value="contain">{__('Contain', 'bitffzc')}</option>
        )}
        {((fldType === 'select' && fields?.[fieldKey]?.mul) || fldType === 'check') && (
          <option value="contain_all">{__('Contain All', 'bitffzc')}</option>
        )}
        {!type.match(/^(date|time|datetime|month|week)$/) && (
          <option value="not_contain">{__('Not Contain', 'bitffzc')}</option>
        )}
        {type === 'number' && <option value="greater">{__('Greater Than', 'bitffzc')}</option>}
        {type === 'number' && <option value="less">{__('Less Than', 'bitffzc')}</option>}
        {type === 'number' && (
          <option value="greater_or_equal">{__('Greater Than or Equal', 'bitffzc')}</option>
        )}
        {type === 'number' && (
          <option value="less_or_equal">{__('Less Than or Equal', 'bitffzc')}</option>
        )}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && (
          <option value="start_with">{__('Start With', 'bitffzc')}</option>
        )}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && (
          <option value="end_with">{__('End With', 'bitffzc')}</option>
        )}
      </MtSelect>
      ...
      <div className="btcd-li-side-btn">
        <Button
          onClick={() => delLogic(lgcGrpInd, lgcInd, subLgcInd, subSubLgcInd)}
          icn
