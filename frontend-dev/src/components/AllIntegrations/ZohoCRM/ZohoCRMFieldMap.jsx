    ...
    >
      <div className="flx integ-fld-wrp">
        <select className="btcd-paper-inp mr-2" name="formField" value={field.formField || ''} onChange={(ev) => handleFieldMapping(ev, i, crmConf, setCrmConf, uploadFields, tab)}>
          <option value="">{__('Select Field', 'bitffzc')}</option>
          {
          ...
          {!uploadFields && <option value="custom">{__('Custom...', 'bitffzc')}</option>}
        </select>

        {field.formField === 'custom' && <MtInput onChange={e => handleCustomValue(e, i, crmConf, setCrmConf, tab)} label={__('Custom Value', 'bitffzc')} className="mr-2" type="text" value={field.customValue} placeholder={__('Custom Value', 'bitffzc')} />}
