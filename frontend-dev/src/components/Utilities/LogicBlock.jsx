...
function LogicBlock({ fieldVal, formFields, fields, delLogic, lgcGrpInd, lgcInd, subLgcInd, subSubLgcInd, value, addInlineLogic, changeLogic, logicValue, changeValue, changeFormField }) {
  ...
  return (
    <div className="flx pos-rel btcd-logic-blk">
      ...
      >
        <option value="">{__('Select Form Field', 'bitffzc')}</option>
      ...
      >
        ...
        <option value="equal">{__('Equal', 'bitffzc')}</option>
        <option value="not_equal">{__('Not Equal', 'bitffzc')}</option>
        <option value="null">{__('Is Null', 'bitffzc')}</option>
        <option value="not_null">{__('Is Not Null', 'bitffzc')}</option>
        {!type.match(/^(date|time|datetime|month|week)$/) && <option value="contain">{__('Contain', 'bitffzc')}</option>}
        {((fldType === 'select' && fields?.[fieldKey]?.mul) || fldType === 'check') && <option value="contain_all">{__('Contain All', 'bitffzc')}</option>}
        {!type.match(/^(date|time|datetime|month|week)$/) && <option value="not_contain">{__('Not Contain', 'bitffzc')}</option>}
        {type === 'number' && <option value="greater">{__('Greater Than', 'bitffzc')}</option>}
        {type === 'number' && <option value="less">{__('Less Than', 'bitffzc')}</option>}
        {type === 'number' && <option value="greater_or_equal">{__('Greater Than or Equal', 'bitffzc')}</option>}
        {type === 'number' && <option value="less_or_equal">{__('Less Than or Equal', 'bitffzc')}</option>}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && <option value="start_with">{__('Start With', 'bitffzc')}</option>}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && <option value="end_with">{__('End With', 'bitffzc')}</option>}
      </MtSelect>
