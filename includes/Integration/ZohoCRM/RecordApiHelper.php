...
use BitCode\BITFFZC\Integration\ZohoCRM\FilesApiHelper;
...
class RecordApiHelper
{
    ...
    public function executeRecordApi($formID, $integId, $defaultConf, $module, $layout, $fieldValues, $fieldMap, $actions, $required, $fileMap = [], $isRelated = false)
    {
            ...
            if (!empty($fieldPair->zohoFormField)) {
                ...
                if (!empty($fieldData[$fieldPair->zohoFormField])) {
                    ...
                    if ($currentLength > $requiredLength) {
                        $error = new WP_Error('REQ_FIELD_LENGTH_EXCEEDED', wp_sprintf(__('zoho crm field %s\'s maximum length is %s, Given %s', 'bitffzc'), $fieldPair->zohoFormField, $module));
        ...
        if (
            ...
            if (!empty($actions->tag_rec) && class_exists('BitCode\BITFFZC\Integration\ZohoCRM\TagApiHelper')) {
                $tags = '';
