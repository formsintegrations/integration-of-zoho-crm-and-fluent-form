...
class Model
{
    ...
    {
        if (is_null($data)) {
            return new WP_Error('empty_data', __('Form data is empty', 'bitffzc'));
        }
    ...
    public function duplicate(array $columns, array $duplicate, array $condition)
    {
        if (!(!\is_null($columns)
            ...
            return new WP_Error(
                'duplicate_error',
                __('Nothing to duplicate', 'bitffzc')
            );
        }
    ...
    public function trash(array $condition = null)
    {
        if (
            ...
            return new WP_Error(
                'deletion_error',
                __('At least 1 condition needed', 'bitffzc')
            );
        }
        $update_condition = (!\is_null($condition) &&
            array_keys($condition) !== range(0, count($condition) - 1)) ? $condition : null;
